...
import reset_vpn
from glpi_client import GlpiClient, GlpiConfig, TransientGlpiError
...
class VpnResilienceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.conn = report_storage.connect(str(Path(self.tmp.name) / "state.db"))
        report_storage.initialize(self.conn)
    ...
    def test_all_vpn_labels_and_inline_fallback(self):
        for label in reset_vpn.VPN_LOGIN_LABELS:
            self.assertEqual(reset_vpn.extract_login_from_ticket_content(f"{label}\njoao.silva"), "joao.silva")
        self.assertEqual(
            reset_vpn.extract_login_from_ticket_content("Usuário(Login) da VPN / Internet joao.silva"),
            "joao.silva",
        )

    def test_state_is_persisted_for_ad_and_glpi_steps(self):
        expiry = datetime(2026, 1, 1, tzinfo=timezone.utc).isoformat()
        report_storage.save_vpn_state(self.conn, 12, "ad_pending", "joao", expiry, "solução")
        state = report_storage.get_vpn_state(self.conn, 12)
        self.assertEqual((state["stage"], state["planned_expiry"], state["glpi_message"]),
                         ("ad_pending", expiry, "solução"))
        report_storage.save_vpn_state(self.conn, 12, "close_pending", "joao", expiry, "solução")
        self.assertEqual(report_storage.get_vpn_state(self.conn, 12)["stage"], "close_pending")
    ...
    def test_incomplete_read_is_identifiable_for_json_and_multipart(self):
        ...
        with patch("glpi_client.urlopen", side_effect=IncompleteRead(b"{", 2)):
    ...
    def test_401_refreshes_session_once(self):
        ...
        with patch("glpi_client.urlopen", side_effect=[error, Response(b'{"session_token":"new"}'), Response(b'{"id":1}')]):
    ...
    def test_resume_does_not_duplicate_solution_and_marks_only_after_close(self):
        ...
        report_storage.save_vpn_state(self.conn, 99, "message_pending", "joao", "date", "solução")
        ...
        reset_vpn._finish_ticket(ticket, glpi, self.conn, solution=True, action="renewed")
        self.assertEqual(report_storage.get_vpn_state(self.conn, 99)["stage"], "completed")
        self.assertTrue(report_storage.was_ticket_processed(self.conn, 99))

    def test_close_failure_keeps_pending_state_and_does_not_mark_action(self):
        ticket = SimpleNamespace(id=100, login="joao")
        report_storage.save_vpn_state(self.conn, 100, "close_pending", "joao", "date", "solução")
        glpi = SimpleNamespace(
        ...
        with self.assertRaises(TransientGlpiError):
            reset_vpn._finish_ticket(ticket, glpi, self.conn, solution=True, action="renewed")
        self.assertEqual(report_storage.get_vpn_state(self.conn, 100)["stage"], "close_pending")
        self.assertFalse(report_storage.was_ticket_processed(self.conn, 100))
